...
const ChangeManagementPanel = ({ changeRequests, setChangeRequests, customerInfo, analysis, baselineData, historicalData, problemStatement }: ChangeManagementPanelProps): React.ReactNode => {
    const [activeRequestId, setActiveRequestId] = useState<string | null>(null);
    const [isSuggesting, setIsSuggesting] = useState<boolean>(false);
    const [requestToDelete, setRequestToDelete] = useState<ChangeRequest | null>(null);
    ...
    const handleSuggestNew = async () => {
        ...
        try {
            const suggestion = await suggestChangeRequest(analysis, baselineData, historicalData, problemStatement);
    ...
    const handleConfirmDelete = () => {
        ...
        setChangeRequests(prev => prev.filter(req => req.id !== requestToDelete.id));
    ...
    return (
        <div id="change-management-panel" className="bg-panel p-6 rounded-lg shadow-md">
                    ...
                    {!activeRequest ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center pt-4 border-t border-border">
                                ...
                                <div className="flex items-center gap-2">
                                    <button 
                                        onClick={() => setRequestToDelete(activeRequest)} 
                                        className="px-3 py-2 bg-white text-danger font-semibold rounded-md border border-danger hover:bg-red-50 transition-colors flex items-center gap-2 text-sm"
            ...
            >
                <p>Are you sure you want to permanently delete the change request titled: <strong className="text-text-primary">"{requestToDelete?.title}"</strong>?</p>
